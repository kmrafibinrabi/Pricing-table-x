...
class vxpt_Column extends vxpt_Type
{
     ...
     * @throws Exception
     */
    public static function createFormArray(array $column_data_array): vxpt_Column
    {
        ...
        if (is_array($column_data_array['feature_data'])) {
            foreach($column_data_array['feature_data'] as $feature_data) {
                $feature_array[] = vxpt_Feature::createFromArray($feature_data);
            }
        }

        return new vxpt_Column($column_id, $column_title, $highlighted, $description, $column_price_currency, $column_price, $column_price_suffix, $column_button_url, $column_button_face_text, $feature_array);
    }
}